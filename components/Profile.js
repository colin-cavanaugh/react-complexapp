...
function Profile() {
  ...
  const [state, setState] = useImmer({
    ...
    profileData: { profileUsername: '...', profileAvatar: 'https://gravatar.com/avatar/placeholder?s=128', isFollowing: false, counts: { postCount: '', followerCount: '', followingCount: '' } },
  })
  // immmediately available when component tries to render.  Update piece of state and react will automatically re render

  // useEffect requires two parameters, first is function, second is list of dependencies we want to watch
  useEffect(() => {
  ...
  }, [state.stopFollowingRequestCount]) // <--- Dependency array
  // empty array tells react to run only when the very first time this component is rendered
